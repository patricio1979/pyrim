...
def organize(
        ...
        for idj,j in enumerate(i): # Go thru each element in the measure
            ...
            if (j.tag == 'forward'):
                ...
                if (fw):
                    hide = 'no'
                    # a. Assign voices
                    voice_num = int(i[idj+voice_id].find('voice').text)
                    staff = (voice_num - 1) // 4
                    voice = (voice_num - 1) % 4
                    ...
                    if staff >= n_staves:
                        print(
                            f"ERROR: voice={voice_num}, staff={staff}, "
                            f"voice_local={voice}, n_staves={n_staves}, measure={measNum}"
                        )
            ...
            if (j.tag == 'note'):
                ...
                if (j.find('chord') is None):
                    ...
                    if rest_elem is not None:
                        # Rests: use the explicit MusicXML duration
                        duration_elem = j.find('duration')

                        if duration_elem is not None:
                            noteType = float(duration_elem.text) * divs
                        else:
                            noteType = 0
                    else:
                        # Regular notes: use rhythmic figure
    ...
    # ---------------------------------------------------------
    # Fix for close open ties and hairpins at the end of the piece in corrupted MusicXML files
    # ---------------------------------------------------------
    for staff_idx in range(n_staves):
        for voice_idx in range(4):

            # ---- TIE ----
            if all_tie[staff_idx][voice_idx]:

                last_tie = all_tie[staff_idx][voice_idx][-1]

                # If the last recorded note has an open tie marker,
                # add a closing marker at the last note.
                if last_tie[1] == 1:
                    all_tie[staff_idx][voice_idx].append(
                        [last_tie[0], 0, last_tie[2]]
                    )

            # ---- WEDGE / HAIRPIN ----
            if all_wedge[staff_idx][voice_idx]:

                last_wedge = all_wedge[staff_idx][voice_idx][-1]

                if last_wedge[1] != '0':
                    all_wedge[staff_idx][voice_idx].append(
                        [last_wedge[0], 'stop', last_wedge[2]]
                    )
